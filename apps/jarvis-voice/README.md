...
For a clean copy-from-scratch checklist, see [`SETUP.md`](SETUP.md).
...
J.A.R.V.I.S. reads environment values from:

1. `.env` in this repo (development and `npm start`).
2. `~/.jarvis/.env` (packaged app on macOS/Linux).
3. `%USERPROFILE%\\.jarvis\\.env` (packaged app on Windows).
...
| Gemini voice | `Leda` | `electron/main.mjs` (`GEMINI_LIVE_VOICE` env override) | Gemini Live prebuilt voices |
| Gemini SDK | `@google/genai` `^2.10.0` | `package.json` | npm |
| Local wake word | `openWakeWord` pretrained `hey_jarvis` model | `scripts/openwakeword-listener.py` + `electron/main.mjs` (`OPENWAKEWORD_*` env overrides) | dscripka/openWakeWord |
...
- npm.
- Python 3.9+ for the local openWakeWord helper.
- Hermes Agent installed and able to run `hermes gateway`.
- A Gemini API key for the Live model (`GEMINI_API_KEY`).
- For local wake-word detection: the ignored `.venv-openwakeword` Python environment created by the setup steps below. No wake-word API key or provider account is required.
- macOS, Windows, or Linux with microphone permission available.
- Optional webcam permission for gesture control. USB/UVC camera-mic devices, including OBSBOT Tail Air, can be used when the OS exposes them as normal camera and microphone inputs.
...
### 2. Install local wake-word runtime

openWakeWord runs locally in an ignored Python venv. This downloads the pretrained `hey_jarvis` model once; runtime wake detection does not use an API key or cloud service.
...
### 3. Configure Gemini and JARVIS

Create `.env` from `.env.example` and set at least `GEMINI_API_KEY`.

### 4. Enable Hermes gateway API

Make sure Hermes API is enabled:

```bash
echo 'API_SERVER_ENABLED=true' >> ~/.hermes/.env
echo 'API_SERVER_KEY=<LOCAL_HERMES_API_KEY>' >> ~/.hermes/.env
hermes gateway restart
```
...
show the generic Electron app name, but the packaged app is named JARVIS.

### 6. Run a production build without packaging
...
- `API_SERVER_KEY` is required and must be a local ignored secret. Do not use a shared default key.
